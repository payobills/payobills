        ...
        .unwrap();
                        if let Some(caps) = re.captures(&record.transaction_text.unwrap()) {
                            ...
                            let time_zone_capture = caps
                                ...
                                .expect("CAPTURE TO BE PRESENT")
                                .as_str();
                            let time_zone_percentage_z_format =
                                timezone_to_offset(time_zone_capture);

                            let full_back_date_capture = format!(
                                "{} {}",
                                date_string_capture, time_zone_percentage_z_format
                            );
                        ...
                        if let Some(caps) = re.captures(&record.transaction_text.unwrap()) {
                            ...
                            let full_back_date_capture =
                                format!("{} 00:00 +0530", date_string_capture);
                            println!("Date string captured {}", full_back_date_capture);
        ...
        .unwrap();
            ...
            match record.currency {
                Some(currency) => {
                    if currency != "INR" {
                        let paid_at =
                            DateTime::parse_from_rfc3339(record.paid_at.clone().unwrap().as_str())
                                .unwrap();
                        ...
                        let exchange_records: NocoDBResponse<HistoricalCurrencyExchangeRateRecord> =
                            get_nocodb_records(
                                ...
                                format!("(Date,eq,exactDate,{})&l=1", transaction_date_string)
                                    .as_str(),
                            )
                            .await?;
                        ...
                        if exchange_records.list.len() > 0 {
                            ...
                            match record.amount {
                                Some(amount) => {
                                    ...
                                    let matching_currency_records: Vec<&CurrencyRecord> =
                                        currencies
                                            .iter()
                                            .filter(|curr| {
                                                currency == curr.code || currency == curr.symbol
                                            })
                                            .clone()
                                            .collect();
                                    ...
                                    if matching_currency_records.len() > 0 {
                                        ...
                                        let normalized_amount = amount
                                            / exchange_rate_usd_to_source
                                            * exchange_rate_usd_to_inr;

                                        changes.insert(
                                            "Currency".to_string(),
                                            Value::Str(
                                                matching_currency_record.code.as_str().to_string(),
                                            ),
                                        );
                                        changes.insert(
            ...
            nocodb_env.table_name_payobills_transactions
                ...
                for transaction in response.list {
                    match transaction.bills {
                        Some(_) => {
                            parse_transaction(transaction.clone(), nocodb_env.clone()).await?
                        }
                        None => {}
                    }
                }
