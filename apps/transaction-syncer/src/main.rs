...
async fn write_transaction_to_nocodb(
    ...
    let nocodb_transcation_table_url = std::env::var("NOCODB_TRANSACTION_TABLE_URL").expect("NOCODB_TRANSACTION_TABLE_URL must be set");
    let resp = client.post(nocodb_transcation_table_url)
    .headers(map)
    .json(&body)
    .send()
    .await?;
