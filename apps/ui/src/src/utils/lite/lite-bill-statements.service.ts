...
export class LiteBillStatementsService implements IBillStatementsService {
	...
	private billStatements = writable<
		Query<{ [key: string]: BillStatementDTO[] } | undefined>
	>({
		fetching: false,
		data: undefined,
		error: null,
	...
	queryBillStatementsByBillIds(
		...
		(async () => {
			try {
				const billStatements: LiteBillStatementDTO[] =
				...
				billIds.forEach(
					(billId) =>
						(billStatementsGroupedByBills[
							`${CONSTANTS.BILL_STATEMENTS_KEY_PREFIX}${billId}`
						] ||= []),
				);
