...
export class NocoDbClient {
  ...
  async getRecord(projectId, tableId, id) {
    if (!projectId || !tableId || !id) {
      throw new Error(
        'Project ID, Table ID and Record ID a from  to fetch a record.'
      );
    }
    ...
    if (res.status !== 200) {
      throw new Error(
        `Failed to fetch record: ${res.status} ${res.statusText}`,
      );
    }
  ...
  async patchRecord(row, id) {
    ...
    if (![200, 201].includes(rowUploadResponse.status)) {
      throw new Error(
        `Row update failed with status ${rowUploadResponse.status}: ${rowUploadResponse.data}`,
      );
    }
  ...
  async putObject(_, fileName, buffer, tags) {
    ...
    if (![200, 201].includes(rowUploadResponse.status)) {
      throw new Error(
        `Row upload failed with status ${rowUploadResponse.status}: ${rowUploadResponse.statusText}`,
      );
    }
